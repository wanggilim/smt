...
class DCS(object):
    ...
    def getAORs(self, aorID, pos=False, guide=False,
                as_table=False, as_pandas=False, as_json=False, raw=False):
        """Get AOR row.
        ...
        if isinstance(aorID,str):
            ...
            aors = self._query_AOR_table(aorID=aorID,pos=pos,guide=guide)
            if not aors:
                return self._getObsPlan(planID, aorID=aorID, raw=raw,
                                        as_table=as_table,as_pandas=as_pandas,as_json=as_json,
                                        pos=pos, guide=guide)
            ...
            if self.refresh_cache:
                ...
                cfiles = [self._getObsPlan(planID,pos=pos,guide=guide,raw=True) for planID in planIDs]
            aors = self._query_AOR_table(aorID=aorID,pos=pos,guide=guide)
            if not aors:
                planIDs = (_aorID_to_planID(a) for a in aorID)
                cfiles = [self._getObsPlan(planID, pos=pos,guide=guide,raw=True) for planID in planIDs]
    ...
    def _query_AOR_table(self, search, sql=False):
        ...
        aors = AOR.select().where((AOR.aorID.in_(aorID))  |
                                  ...
                                  (AOR.ObsBlk.in_(aorID))).order_by(AOR.ObsBlk,AOR.order,AOR.aorID)
        if sql:
            return aors
        aors = list(aors.dicts())
        aors = aors if aors else None
        return aors
    ...
    def _query_POS_table(aorID=None,planID=None,target=None,guide=False):
        if aorID:
            if isinstance(aorID,str):
                ...
                if _is_aorID(aorID):
                    if guide:
                        pos = POS.select(POS,GUIDE).join(GUIDE).switch(POS).where(POS.AORID == aorID).dicts()
                    else:
               ...
               # pos = pos if pos else None
                    ...
                    planID = _ObsBlk_to_planID(aorID)
                    return DCS._query_POS_table(planID=planID,guide=guide)
                else:
                    pos = None
            else:
                if guide:
                    pos = POS.select(POS,GUIDE).join(GUIDE).switch(POS).where((POS.AORID.in_(aorID)) | (POS.planID.in_(aorID))).dicts()
                else:
                    pos = POS.select().where((POS.AORID.in_(aorID)) | (POS.planID.in_(aorID))).dicts()
            ...
            if isinstance(planID,str):
                # single planID
                if guide:
                    pos = POS.select(POS,GUIDE).join(GUIDE).switch(POS).where(POS.planID == planID).dicts()
                else:
            ...
            if isinstance(target,str):
                # single target
                if guide:
                    pos = POS.select(POS,GUIDE).join(GUIDE).switch(POS).where(POS.Target == target).order_by(POS.POSName).dicts()
                else:
                    pos = POS.select().where(POS.Target == target).order_by(POS.POSName).dicts()
                    
            else:
                if guide:
                    pos = POS.select(POS,GUIDE).join(GUIDE).switch(POS).where(POS.Target.in_(target)).order_by(POS.POSName).dicts()
                else:
    ...
    def _getObsPlan(self, planID,
        ...
        if pos:
            self.getPOS(planID,guide=guide, raw=True)
            
        if raw:
            return cfile

        aors = self._query_AOR_table(aorID=aorID,ObsBlk=ObsBlk,planID=planID, pos=pos, guide=guide)
